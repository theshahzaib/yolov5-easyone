...
# Image Window Size
window_size = [416,416]
# GPUs for training 'cpu' if training on CPU
gpu_count = 'cpu'
# Batch
batch = 16
# Epochs
...
f.write('python train.py --img '+str(window_size[0])+' --batch '+str(batch)+' --epochs '+str(epochs)+' --data ./coco.yaml --cfg yolov5s.yaml --weights yolov5s.pt --name '+dataset_folder_name+' --device '+gpu_count+'\n')
f.write('# python train.py --img '+str(window_size[0])+' --batch '+str(batch)+' --epochs '+str(epochs)+' --data ./coco.yaml --cfg yolov5m.yaml --weights yolov5m.pt --name '+dataset_folder_name+' --device '+gpu_count+'\n')
f.write('# python train.py --img '+str(window_size[0])+' --batch '+str(batch)+' --epochs '+str(epochs)+' --data ./coco.yaml --cfg yolov5l.yaml --weights yolov5l.pt --name '+dataset_folder_name+' --device '+gpu_count+'\n')
f.write('# python train.py --img '+str(window_size[0])+' --batch '+str(batch)+' --epochs '+str(epochs)+' --data ./coco.yaml --cfg yolov5x.yaml --weights yolov5x.pt --name '+dataset_folder_name+' --device '+gpu_count+'\n')
f.close()
